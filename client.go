...
import (
	...
	"github.com/andock/ssh2docksal/pkg/envhelper"
	"github.com/andock/ssh2docksal/pkg/ttyhelper"
	"github.com/apex/log"
...
func NewClient(conn *ssh.ServerConn, chans <-chan ssh.NewChannel, reqs <-chan *ssh.Request, server *Server) *Client {
	...
	log.Infof("Accepted %s for %s from %s port %s ssh2: %s", "public", conn.User(), remoteAddr[0], remoteAddr[1], "")
	return &client
}
...
func (c *Client) runCommand(channel ssh.Channel, entrypoint string, command []string) {
	...
	cmd = exec.Command("docker", "ps", "--filter=label=ssh2docksal", fmt.Sprintf("--filter=label=image=%s", "c.Config.ImageName"), fmt.Sprintf("--filter=label=user=%s", c.Config.RemoteUser), "--quiet", "--no-trunc")
