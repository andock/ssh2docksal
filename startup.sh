...
uid_gid_reset ()
{
	if [[ "$HOST_UID" != "$(id -u docker)" ]] || [[ "$HOST_GID" != "$(id -g docker)" ]]; then
		...
		sudo usermod -u "$HOST_UID" -o docker
		sudo groupmod -g "$HOST_GID" -o "$(id -gn docker)"
	fi
}
...
sudo chown "${HOST_UID:-1000}:${HOST_GID:-1000}" -R "$HOME_DIR"

/go/src/github.com/andock/ssh2docksal/ssh2docksal $*