...
// DockerCleanup cleans all containers created by ssh2docksal
...
// DockerListContainers lists containers created by ssh2docksal
func DockerListContainers(all bool) ([]string, error) {
	command := []string{"docker", "ps", "--filter=label=ssh2docksal", "--quiet", "--no-trunc"}
	if all {
